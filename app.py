...
def matchs_du_jour(jours, explorer):
    ...
    return sorted(sortie, key=lambda x: x["quand"])
...
if mode == "Matchs du jour":
    ...
    explorer = c2.checkbox("Explorer toutes les competitions", value=False,
                           help="Plus complet mais beaucoup plus lent : "
                                "chaque competition est parcourue.")
    if c3.button("Actualiser la liste", use_container_width=True):
        matchs_du_jour.clear()

    with st.spinner("Lecture de Betclic..."):
        try:
            liste = matchs_du_jour(jours, explorer)
