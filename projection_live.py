...
# Correction du biais de projection. Quand une regle ne se declenche pas, le
# modele applique zero au lieu de la deviation inverse : les projections
# derivent vers le haut (mesure : +0.92 tir sur les totaux). Avec True, chaque
# famille contribue toujours, et la projection est centree sur la base.
# A ACTIVER DANS LES DEUX OUTILS A LA FOIS, puis refaire le backtest : les
# probabilites changent, donc les cellules de validation aussi.
CENTRER_PROJECTIONS = True
...
def delta_centre(regles, valeurs, cible):
    """
    Delta corrige : chaque famille contribue, qu'elle se declenche ou non.
    Famille declenchee -> deviation de sa regle la plus forte.
    Famille non declenchee -> deviation INVERSE de sa regle la plus forte.
    La moyenne des corrections vaut alors zero sur l'ensemble des matchs.
    """
    sous = regles[regles.cible == cible]
    if not len(sous) or "dev_inverse" not in sous.columns:
        return None, []
    contributions = []
    for fam, g in sous.groupby(sous.variable.map(famille_de)):
        g = g.reindex(g.ecart_moyenne.abs().sort_values(ascending=False).index)
        actives = []
        for _, r in g.iterrows():
            ...
            if col not in valeurs or valeurs[col] is None:
                continue
            v = valeurs[col]
            if (v > r.seuil) if r.sens == ">" else (v <= r.seuil):
                actives.append(r)
        if actives:
            contributions.append((fam, float(actives[0].ecart_moyenne), True))
        else:
            ref = g.iloc[0]
            if pd.notna(ref.dev_inverse):
                contributions.append((fam, float(ref.dev_inverse), False))
    if not contributions:
        return None, []
    return float(np.mean([c[1] for c in contributions])), contributions
...
def bloc_html(cible, sous, valeurs, infos, moyenne, sd=None,
              detail_base=None, delta_corrige=None):
    quoi, qui = LIBELLE_CIBLE.get(cible, (cible, ""))
    dom = infos.get("dom", "Domicile")
    ext = infos.get("ext", "Exterieur")
    ...
    if CENTRER_PROJECTIONS and delta_corrige is not None:
        delta = delta_corrige                  # projection centree
...
def rapport_html(res, valeurs, infos, regles, chemin="projection.html"):
    ...
    for cible in cibles:
        ...
        if col_cible:
            ...
            moy = moy_eq
        dc = (delta_centre(regles, valeurs, cible)[0]
              if CENTRER_PROJECTIONS else None)
        h, info_bloc = bloc_html(cible, sous, valeurs, infos, moy,
                                 sds.get(cible), detail_base, dc)
...
{note_inj}</div>
<div class="valinfo" title="{'projections centrees' if CENTRER_PROJECTIONS else 'projections brutes'}" style="{'border-left-color:#ff6b6b;color:#ff9d9d'
...
def main():
    ...
    # Le match peut etre passe en argument : permet un raccourci qui lance
    # directement la projection sans rien taper.
    #   python projection_live.py "atl madrid"
    if len(sys.argv) > 1:
        entree = " ".join(sys.argv[1:]).strip()
        print(f"\nMatch demande : {entree}")
    else:
