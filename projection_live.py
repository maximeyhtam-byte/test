...
VALIDATION = {           # (ecart au temoin en points, nombre de paris testes)
    ("buts", "domicile"):     (+18.1, 86),
    ("tirs", "domicile"):     (+8.3, 735),
    ("cadres", "domicile"):   (+6.1, 666),
    ("tirs", "exterieur"):    (-2.4, 937),
    ("cadres", "exterieur"):  (-21.4, 36),
    ("buts", "exterieur"):    (None, 1),
}
ECART_MINIMUM_VALIDE = 3.0   # points d'ecart au temoin exiges
INFO_VALIDATION = "valeurs ecrites en dur (backtest du 20/08/2026)"
...
def charger_validation():
    ...
    """
    global VALIDATION, ETOILES_MIN, INFO_VALIDATION
    ...
    try:
        ...
        if not len(v):
            return False
        VALIDATION = {(str(l.quoi), str(l.qui)): (float(l.ecart_points),
                                                  int(l.n_paris))
...
def verdict_marche(quoi, qui, etoiles):
    ...
    ecart, n = VALIDATION.get((quoi, qui), (None, 0))
    ...
    if ecart < ECART_MINIMUM_VALIDE:
        return ("rejet", f"Marche NON RENTABLE au backtest : {ecart:+.1f} points "
                f"vs strategie naive, sur {n} paris. S'abstenir.", ecart)
    if etoiles < ETOILES_MIN:
        return ("faible", f"Marche valide ({ecart:+.1f} pts sur {n} paris) mais "
                f"{etoiles} etoile(s) seulement : en dessous de {ETOILES_MIN}, "
                f"l'avantage devient marginal.", ecart)
    return ("ok", f"Marche valide par le backtest : {ecart:+.1f} points vs "
            f"strategie naive, sur {n} paris. {etoiles} etoiles.", ecart)
PARSER = "lxml"
...
def bloc_total_derive(quoi, i_dom, i_ext, valeurs, infos):
    """
    Total reconstruit par somme des deux equipes.
    Utile surtout pour les buts, ou aucune regle ne porte sur le total :
    le processus y est quasi poissonien, mais chaque camp a ses propres regles.
    """
    if i_dom is None or i_ext is None:
        return None, None
    dom, ext = infos.get("dom", "Domicile"), infos.get("ext", "Exterieur")
    moyenne = i_dom["moyenne"] + i_ext["moyenne"]
    proj = i_dom["proj"] + i_ext["proj"]
    delta = proj - moyenne

    # on met en commun les familles des deux camps : toutes contribuent au total
    parts = []
    for cle, info in (("dom", i_dom), ("ext", i_ext)):
        b = info.get("best")
        equipe = dom if cle == "dom" else ext
        if b is None or not len(b):
            continue
        for _, l in b.iterrows():
            parts.append({"famille": f"{l.famille} ({equipe})", "ecart": l.ecart})
    if not parts:
        return None, None
    best = pd.DataFrame(parts)
    ...
    lignes = ""
    for _, l in best.reindex(best.ecart.abs().sort_values(ascending=False).index).iterrows():
        cls = "up" if l.ecart > 0 else "down"
        lignes += (f'<tr><td><span class="fam">{l.famille}</span></td>'
                   f'<td class="e {cls}">{l.ecart:+.2f}</td></tr>')

    base = {"tirs": "mt_tirs", "cadres": "mt_cadres", "buts": "mt_score"}.get(quoi)
    deja = None
    if base:
        a, b_ = valeurs.get(base + "_dom"), valeurs.get(base + "_ext")
    ...
    _CID[0] += 1
    cid_t = _CID[0]
    html = (f'<div class="card s{note} total">'
            f'<div class="hd"><h2>{quoi} 2e mi-temps &mdash; TOTAL '
            f'(somme des deux equipes)</h2></div>{v}'
            f'<div class="duo"><div class="proj primaire">'
            f'<div class="ptit">total reconstruit'
            f'<span class="info" title="Somme des projections domicile et '
            f'exterieur. Chaque camp a ses propres regles ; leur somme donne '
            f'le total, meme quand aucune regle ne porte directement dessus.">'
            f'somme</span></div>'
            f'<div class="num {sens}">{proj:.2f}</div>'
            f'<div class="pdelta {"up" if delta>0 else "down"}">{delta:+.2f} '
            f'vs moyenne {moyenne:.2f}</div>'
            f'<div class="pnote">{i_dom["proj"]:.2f} ({dom}) + '
            f'{i_ext["proj"]:.2f} ({ext})</div></div>'
            f'<div class="chart" style="flex:1.4"><h3>contributions cumulees</h3>'
            f'{svg_familles(best, dom, ext)}</div></div>'
            f'{bloc_total}'
            f'{bandeau_verdict(quoi, "total", note)[0]}'
            f'{bloc_cotes(cid_t, proj, deja, quoi, "prudence")}'
            f'<table class="rules"><tr><th>famille (equipe)</th>'
            f'<th style="text-align:right">ecart</th></tr>{lignes}</table>'
            f'<div class="range">Les deux camps sont additionnes : un ecart '
            f'positif d un cote et negatif de l autre s annulent, ce qui fait '
            f'chuter la note.</div></div>')
    return html, {"cible": f"{quoi} total derive", "quoi": quoi,
                  "equipe": "les deux equipes (somme)", "proj": proj,
                  "moyenne": moyenne, "etoiles": note,
...
BARRE_REGLAGES = """
<div class="reglages">
  <label>Bankroll <input type="number" id="bankroll" value="100" step="10"> EUR</label>
  ...
  tiennes ne le sont pas : une fraction protege contre l exces de confiance.</span>
</div>
"""
...
def bloc_html(cible, sous, valeurs, infos, moyenne, sd=None,
    ...
    if sous is None or not len(sous):
        ...
        return html, {"cible": cible, "quoi": quoi, "equipe": equipe,
                      ...
                      "total_match": None, "best": None, "delta": 0.0,
                      "cote": qui}
...
def rapport_html(res, valeurs, infos, regles, chemin="projection.html"):
    ...
    for cible in cibles:
        ...
        if moy is None:
            continue
        if qui_total(cible) and (sous is None or not len(sous)):
            continue  # un total sans regle est remplace par le total derive
    ...
    # totaux reconstruits par somme des deux camps
    for quoi in ("tirs", "cadres", "buts"):
        h, info = bloc_total_derive(quoi, par_cote.get((quoi, "domicile")),
                                    par_cote.get((quoi, "exterieur")),
                                    valeurs, infos)
        if h:
            blocs += h
            resume.append(info)
...
{note_inj}</div>
<div class="valinfo">Validation des marches : {INFO_VALIDATION}</div>
{BARRE_REGLAGES}
